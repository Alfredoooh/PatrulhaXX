...
class LockScreen extends StatefulWidget {
  ...
  const LockScreen({super.key, this.mode = LockMode.unlock,
      this.onUnlocked, this.onPinSet});
...
class _LockScreenState extends State<LockScreen> with TickerProviderStateMixin {
  ...
  void _onKey(String k) {
    if (_processing || _input.length >= _pinLength) return;
  ...
  void _onDel() {
    if (_processing || _input.isEmpty) return;
    HapticFeedback.lightImpact();
  ...
  Future<void> _onConfirm() async {
    ...
    if (_input.length < _pinLength) { _triggerError(); return; }
  ...
  @override Widget build(BuildContext context) {
    ...
    // modo escuro: teclas numéricas têm fundo ligeiramente diferente
    final keyBg         = isDark ? const Color(0xFF2C2C2E) : const Color(0xFFFFFFFF);
    // teclas de ação (olho, apagar) ficam com o mesmo keyBg no escuro
    final actionKeyBg   = isDark ? const Color(0xFF2C2C2E) : keypadBg;
            ...
            children: [
              SizedBox(
                ...
                child: Lottie.asset('assets/lottie/lock.json',
                  ...
                  errorBuilder: (_, __, ___) => Icon(Icons.lock_rounded,
                    size: 56,
                    color: isDark
                              ...
                              : filled
                                  ? Container(width: 10, height: 10,
                                      decoration: BoxDecoration(
                                        ...
                                        color: _error ? AppTheme.error : titleColor))
                                  : null),
                          ),
                        ),
                      );
...
class _Keypad extends StatelessWidget {
  ...
  final Color keyBg;
  final Color actionKeyBg; // fundo igual ao numérico no dark
  final Color keyText;
  ...
  // ── Altura das linhas ligeiramente reduzida ──
  static const double _rowH = 64.0;

  @override Widget build(BuildContext context) {
    final botPad = MediaQuery.of(context).padding.bottom;
    final iconColor = keyText.withOpacity(0.65);
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        AnimatedContainer(
          ...
          child: LinearProgressIndicator(
            ...
            valueColor: AlwaysStoppedAnimation<Color>(keyText.withOpacity(0.15))),
        ),
        ...
        // Última linha: olho | 0 | apagar
        SizedBox(
          height: _rowH,
          child: Row(children: [
            // Olho — mesmo fundo das numéricas
            Expanded(child: _KeyBtn(
              keyBg: actionKeyBg,
              onTap: onToggleVisible,
              enabled: !processing,
              child: _SvgIcon(
                path: isVisible
                    ? 'assets/icons/svg/visibility_off.svg'
                    : 'assets/icons/svg/visibility.svg',
                color: iconColor,
                fallback: Icon(
            ...
            Expanded(child: _KeyBtn(
              ...
              child: Text('0', style: TextStyle(
                color: keyText, fontSize: 26, fontWeight: FontWeight.w400)),
            )),
            Container(width: 1, height: _rowH, color: dividerColor),
            // Apagar — mesmo fundo das numéricas
            Expanded(child: _KeyBtn(
              keyBg: actionKeyBg,
              onTap: onDelete,
              enabled: !processing,
              child: _SvgIcon(
                path: 'assets/icons/svg/backspace.svg',
                color: iconColor,
                fallback: Icon(Icons.backspace_outlined, color: iconColor, size: 22),
              ),
            )),
          ]),
        ),
    ...
    return SizedBox(
      ...
      child: Row(children: [
        for (int i = 0; i < keys.length; i++) ...[
          if (i > 0) Container(width: 1, height: _rowH, color: dividerColor),
          Expanded(child: _KeyBtn(
            keyBg: keyBg,
            onTap: () => onKey(keys[i]),
            enabled: !processing,
...
class _SvgIcon extends StatelessWidget {
  ...
  const _SvgIcon({required this.path, required this.color, required this.fallback});
...
class _KeyBtn extends StatefulWidget {
  ...
  final Widget child;
  final bool enabled;
  const _KeyBtn({required this.keyBg, required this.onTap,
      required this.child, this.enabled = true});
  @override State<_KeyBtn> createState() => _KeyBtnState();
}
  ...
  @override Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        color: _pressed ? widget.keyBg.withOpacity(0.45) : widget.keyBg,
