...
class _SettingsPageState extends State<SettingsPage>
  ...
  String get _activeIconAsset {
    final data = _kIcons.firstWhere(
      (d) => d.variant == _activeIcon,
  ...
  void initState() {
    ...
    AppIconService.getActiveIcon().then((v) {
      if (mounted) setState(() => _activeIcon = v);
    });
  }
  ...
  void _applySecure(bool v) {
    try {
      _secureChannel.invokeMethod('setSecure', {'enable': v});
    } catch (_) {}
  }

  Future<T?> _openIosSheet<T>({
    required WidgetBuilder builder,
    double maxHeightFactor = 0.58,
  }) async {
    if (_sheetBusy) return null;
    _sheetBusy = true;
