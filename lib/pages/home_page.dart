...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _fadeIn = AnimationController(vsync: this,
  ...
  @override
  void dispose() { _searchCtrl.dispose(); _fadeIn.dispose(); super.dispose(); }
          ...
          child: SafeArea(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Padding(
                ...
                child: Row(children: [
                  _CircleBtn(svg: _svgDownload,
                      onTap: () => Navigator.push(context, _up(const DownloadsPage()))),
                  const Spacer(),
                  _CircleBtn(svg: _svgSettings,
                      onTap: () => Navigator.push(context, _up(const SettingsPage()))),
                ]),
              ),
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  if (_feedLoading)
                    ...
                    GestureDetector(
                      onTap: () { FeedService.instance.load(force: true).then((_) {
              ...
              Expanded(
                child: _feedLoading
                    ...
                    : _feed.isEmpty
                        ? Center(child: Text('Sem resultados de feed',
          ...
          ClipRRect(
            ...
            child: Stack(children: [
              item.thumb.isNotEmpty
                  ? CachedNetworkImage(
                      imageUrl: item.thumb, width: 150, height: 90, fit: BoxFit.cover,
                      placeholder: (_, __) => Container(width: 150, height: 90,
                          color: Colors.white.withOpacity(0.05)),
                      errorWidget: (_, __, ___) => Container(width: 150, height: 90,
                          color: Colors.white.withOpacity(0.05),
                          child: const Icon(Icons.play_circle_outline, color: Colors.white24)),
                    )
                  : Container(width: 150, height: 90, color: Colors.white.withOpacity(0.05),
                      child: const Icon(Icons.play_circle_outline, color: Colors.white24)),
              if (item.duration.isNotEmpty)
                Positioned(bottom: 4, right: 6,
                  child: Container(
                    ...
                    child: Text(item.duration, style: const TextStyle(color: Colors.white, fontSize: 9.5)),
                  )),
              Positioned(top: 4, left: 6,
                child: Container(
                  ...
                  decoration: BoxDecoration(color: Colors.black.withOpacity(0.7),
                      borderRadius: BorderRadius.circular(4)),
                  child: Text(item.source, style: const TextStyle(color: Colors.white60, fontSize: 9)),
                )),
            ]),
          ),
          const SizedBox(height: 5),
          Text(item.title, maxLines: 2, overflow: TextOverflow.ellipsis,
              style: TextStyle(color: Colors.white.withOpacity(0.65), fontSize: 11, height: 1.3)),
        ]),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return BrowserPage(
      site: SiteModel(id: 'feed', name: title, baseUrl: url,
          allowedDomain: '', searchUrl: url, primaryColor: const Color(0xFF1A1A1A)),
      freeNavigation: true,
    );
  }
}
